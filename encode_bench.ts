...
if (import.meta.main) {
  ...
  console.log("raw");
  console.log(
    `  BONE ${bone_size} bytes (${(bone_size / 1024).toFixed(2)} KB)`,
  );
  console.log(
    `  JSON ${json_size} bytes (${(json_size / 1024).toFixed(2)} KB) (${
      raw_ratio.toFixed(2)
    }x larger than BONE)`,
  );
  console.log("gzip");
  console.log(
    `  BONE ${bone_gzip_size} bytes (${(bone_gzip_size / 1024).toFixed(2)} KB)`,
  );
  console.log(
    `  JSON ${json_gzip_size} bytes (${
      (json_gzip_size / 1024).toFixed(2)
    } KB) (${gzip_ratio.toFixed(2)}x larger than BONE)`,
  );
  console.log("---");
}
